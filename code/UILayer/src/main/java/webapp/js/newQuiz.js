...
function changeContent(opt) {
	...
	if (opt == "QuestVF") {
		...
		document.getElementById("questionWrapped").innerHTML += '<div class="question wrapped" id="' + opt + 'Wrapped' + contatore + '">' +
			'<button class="deleteSingleQuestion" onclick="deleteQuest2(this.id)" id="' + contatore + '"><img class="bin" src="/images/rubbish.png" ></button>' +
			'<p style="padding:20px" class="titleQ" id="titleQ' + contatore + '">Question#' + contatore + ": <br>" + output[0] + '</p><p class="p-answer">' +
			'</p><p class="p-answer is-padded">Answer:</p><br><p class="p-answer is-padded is-answer" id="' + contatore + 'ans1">&#8226;' + output[1] +
			'</p><br><p class="p-answer is-padded is-answer" id="' + contatore + 'ans2"> &#8226;' + output[2] + '</p><br>' +
...
function uploadData() {
	var count = confirmedQuest.length;
	...
	for (var i = 0; i < count; i++) {
		var domanda = confirmedQuest[i];
		var rispostaCorretta = confirmedRigthAns[i];
		var risposte = confirmedAnsw[i];

		if (risposte[2] == null && risposte[3] == null) {
			data.domande.push({
				tipo: 0,
				domanda: domanda,
				rispostaCorretta: rispostaCorretta == "True" ? "vero" : "falso"
			});
		} else {
			data.domande.push({
				tipo: 1,
	...
	if(data.titolo=="" || data.titolo==null ){
		alert("Inserisci il titolo del quiz");
	...
	fetch("/home/ap/creaQuiz.jsp", {
		...
		console.log(resp);
		if (resp.ok) {
			window.location.replace("/home/dashboard.jsp");
		} else {
			document.getElementById("confirmbutton").disabled = false;
			confirmedAnsw = [];
			confirmedQuest = [];
